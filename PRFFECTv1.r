...
library(lattice)     # randomForest auto-attaches these.  Sometimes there's a masking problem, so have just explicitly loaded these.
library(ggplot2)     # ^
...
library(prospectr) #implements derivatives with SG smoothing
...
get_tr_te_split<-function(f=NULL, y=NULL, patients=patients){
  ...
  if(length(all)!=dim(f)[1]){
    stop("ERROR! Training and test sets have wrong number of spectra in them")
  }
...
#Here processed data appended onto the end of the starting dataset
